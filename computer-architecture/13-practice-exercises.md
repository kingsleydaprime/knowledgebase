...
**A warning that is itself the lesson: several of these fail to reproduce at `-O2`.** That isn't a broken exercise — modern compilers defeat the classic demonstrations, and finding out *how* teaches more than the original result. Solutions in [[computer-architecture/14-practice-exercises-solutions|note 14]].
...
**Done when:** you can state how many significant decimal digits a double actually carries, and why the failure begins exactly where it does → [[computer-architecture/02-data-representation|note 02]].
...
**Done when:** the sanitiser has reported something, and you can say which of the three is defined behaviour and which is UB → [[computer-architecture/02-data-representation|note 02]] · [[languages/04-c/index|C]].
...
**Done when:** you've watched a loop become a constant, and can point at the instruction that replaced it → [[computer-architecture/04-assembly|note 04]].
...
**Done when:** the plot has visible **steps**, and the positions of those steps match the L1/L2/L3 sizes you wrote down. **This is the single best exercise in this course** — you will have measured the memory hierarchy without being told it exists → [[computer-architecture/08-the-memory-hierarchy|note 08]].
...
**Done when:** you can explain why per-element cost rises until stride 16 (on 64-byte lines with 4-byte ints) and then stops rising → [[computer-architecture/09-caches-in-depth|note 09]].
...
**Done when:** you have three timings and can explain the difference **purely in terms of access patterns**, without mentioning instruction count. Try N=512 as well — **the effect changes, and why is the point** → [[computer-architecture/09-caches-in-depth|note 09]].
...
**Done when:** you have a large difference at `-O0` and **almost none at `-O2`**, and you have found the instruction in the `-O2` assembly that explains it. **This is the most instructive exercise here** → [[computer-architecture/07-branch-prediction-and-speculation|note 07]].
...
**Done when:** the padded version is **substantially faster despite using more memory**, and you can name the phenomenon → [[computer-architecture/11-multicore-and-memory-models|note 11]].
...
**Done when:** you have observed a non-zero count on x86, or explained why you didn't — then inserted a fence and watched it go to zero → [[computer-architecture/11-multicore-and-memory-models|note 11]].
...
```
**Done when:** you can state your IPC, whether it's good for this workload, and **which counter you'd look at next** to find out why. IPC alone is not a verdict → [[computer-architecture/12-performance|note 12]].
...
## Related
- [[computer-architecture/14-practice-exercises-solutions|Solutions]] — with measured results
- [[computer-architecture/index|the course]]
- [[gpu-and-parallel-computing/08-practice-exercises|GPU exercises]] — the same method, other hardware
