...
The big three (AWS/Azure/GCP) offer the same _categories_ of service under different names — learn the category and the specific product name is a lookup.
...
- **You** are always responsible for your data, your access controls (IAM), your network config (leaving a security group open to the world is _your_ breach, not theirs), and — for IaaS — patching the guest OS.

Most cloud breaches are not the provider being hacked; they're a misconfigured S3 bucket left public or an over-permissive IAM role. This is why [[devops/09-secret-management/index|secret management]] and least-privilege IAM matter so much.
...
- **Egress (data transfer out)** is charged, often heavily; ingress is usually free. Moving data _between_ regions or _out_ to the internet is a common bill shock.
...
- [[devops/05-orchestration/index|Orchestration]] — managed Kubernetes on these providers
