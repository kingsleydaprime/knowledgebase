...
[[continuum-mechanics/06-conservation-laws|The balance laws]] give four equations for ten unknowns. The gap is closed by a **constitutive equation** — a relation between stress and deformation that describes *this particular material*.
...
- **A law cannot depend on $\mathbf{F}$ directly** — it must go through $\mathbf{C} = \mathbf{F}^T\mathbf{F}$ or $\mathbf{b} = \mathbf{F}\mathbf{F}^T$, where the rotation cancels. → [[continuum-mechanics/04-strain-measures|Strain Measures]]
...
The Clausius–Duhem inequality must hold for **every** conceivable deformation path. → [[continuum-mechanics/06-conservation-laws|note 06]]
...
That's not a convenience, it's forced. And it's remarkably restrictive: it takes the 81 components of a general fourth-order stiffness tensor down to **two independent constants**. → [[continuum-mechanics/08-linear-elasticity|Linear Elasticity]]
...
Valid for small strain, and enormously useful precisely because it's linear — superposition works, and you can solve one load case and add. → [[continuum-mechanics/08-linear-elasticity|Linear Elasticity]]

**Hyperelastic** — for large elastic strain, defined by a stored-energy function $W$:

$$\mathbf{S} = 2\frac{\partial W}{\partial \mathbf{C}}$$

Deriving stress from a potential is what makes the material **path-independent and non-dissipative by construction** — it satisfies the second law automatically. → [[continuum-mechanics/10-finite-deformation|Finite Deformation]]
...
Stress proportional to **rate** of deformation. Water, air, thin oils, and most gases. → [[continuum-mechanics/11-viscous-fluids-and-navier-stokes|Navier–Stokes]]
...
**von Mises** is the standard for metals, built on the second deviatoric invariant. → [[continuum-mechanics/12-failure-and-yield|Failure and Yield]]
...
## Related
- [[continuum-mechanics/06-conservation-laws|Conservation Laws]] — the equations this closes
- [[continuum-mechanics/08-linear-elasticity|Linear Elasticity]] — the simplest case, in full
- [[continuum-mechanics/12-failure-and-yield|Failure and Yield]] — yield criteria in depth
