...
- [[cybersecurity/README|Cybersecurity]] — a full track (14 sections): fundamentals (incl. **security principles & threat modelling**) → ethical hacking → network/web security → cryptography → attacks taxonomy → security operations (blue team) → GRC → cloud, then the deep technical disciplines — **binary exploitation, Active Directory, malware/RE, API security** — plus **[[cybersecurity/10-protecting-yourself/README|protecting yourself]]**, a jargon-free personal-security course you could send to family
...
- [[mobile/README|Mobile]] — **a full track (Sep 2026)**: what makes mobile different (**the OS can kill your process**), the lifecycle, declarative UI, offline-first, permissions, release with no rollback, native vs cross-platform, plus `frameworks/` (iOS, Android, Flutter, React Native) and the languages [[languages/08-swift/README|Swift]], [[languages/09-kotlin/README|Kotlin]], [[languages/10-dart/README|Dart]]
...
- [[foundations/digital-signal-processing/README|Digital Signal Processing]] — **added Sep 2026**: sampling & Nyquist → the frequency domain → the FFT → **convolution (which is also a CNN's core)** → filters → spectral analysis → modulation & software-defined radio. Bridges hardware/RF, information theory, and computer vision — and the reps run on a **$30 radio dongle**
