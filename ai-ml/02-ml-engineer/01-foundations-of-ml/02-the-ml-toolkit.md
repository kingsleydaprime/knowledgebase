...
**[reference]** — the Python stack every ML engineer works in. roadmap.sh lists these across both the `machine-learning` and `mlops` roadmaps; they're also the shared foundation the [[ai-ml/01-data-scientist/index|Data Scientist]] path needs (noted as a gap in [[ai-ml/00-foundations/index|00-foundations]]).
...
| Library | Role |
|---|---|
| **NumPy** | the foundation — n-dimensional arrays and vectorized math ([[ai-ml/00-foundations/03-mathematics/01-linear-algebra/index\|linear algebra]] in code); nearly everything else is built on it |
| **pandas** | tabular data — the `DataFrame`, for loading, cleaning, and manipulating structured data ([[ai-ml/02-ml-engineer/02-working-with-data/index\|working with data]]) |
...
| **scikit-learn** | classical ML — a consistent API for [[ai-ml/02-ml-engineer/03-classical-ml/index\|regression, trees, SVMs, clustering, PCA]], plus preprocessing, metrics, and model selection. The workhorse for everything that isn't deep learning |
