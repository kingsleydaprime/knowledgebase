...
That last one matters — **a pure time delay is $e^{-sT}$**, which is not a rational function, and it's why delay is so destructive to stability. → [[control-theory/06-frequency-response|Frequency Response]]
...
That's the criterion the entire field rests on. → [[control-theory/05-stability-and-root-locus|Stability]]
...
**$L = GH$ is the loop gain**, and it's the central object of classical design. Note the closed-loop poles are the roots of $1 + L(s) = 0$ — **the characteristic equation.** Everything in [[control-theory/05-stability-and-root-locus|root locus]] and [[control-theory/07-nyquist-and-robustness|Nyquist]] is about the roots of that one equation.

And the insensitivity result from [[control-theory/01-what-control-theory-is|note 01]] falls straight out: when $|L| \gg 1$, $T \approx 1$ regardless of $G$.
...
**Some nonlinearities cannot be linearised away** — saturation, backlash, friction, hysteresis. Those need [[control-theory/13-nonlinear-and-modern-control|nonlinear methods]].
...
**Frequency sweep** — inject sinusoids across a range and measure gain and phase. Gives the Bode plot directly and is the most informative method, at the cost of time. → [[control-theory/06-frequency-response|Frequency Response]]
...
- [[control-theory/03-time-response|Time Response]] — what these poles produce
- [[control-theory/05-stability-and-root-locus|Stability and Root Locus]] — the characteristic equation
- [[control-theory/08-state-space|State Space]] — the alternative representation
