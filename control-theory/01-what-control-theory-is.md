...
**Source:** `[reference]` — see [[control-theory/index|the domain note]].
...
**1. It can make a stable system unstable.** The central risk. Feedback plus delay plus gain equals oscillation — the shower where you overcorrect the temperature and oscillate hot/cold forever. → [[control-theory/05-stability-and-root-locus|Stability]]
...
**4. Performance and robustness trade off.** Making a loop fast makes it less tolerant of model error. **This is not an engineering limitation you can design away** — it's a mathematical constraint. → [[control-theory/07-nyquist-and-robustness|Nyquist and Robustness]]
...
**Actuator limits.** Every real actuator saturates. A design demanding 500 N from a 100 N motor is not a design. → [[control-theory/04-pid-control|PID Control]]
...
Coupling is what makes MIMO genuinely harder — you can't tune loops independently, because each one disturbs the others. Classical transfer-function methods struggle; **state-space methods handle MIMO naturally**, which is the main reason they exist. → [[control-theory/08-state-space|State Space]]
...
- **Computing** — TCP congestion control is a distributed feedback loop, and autoscaling is a controller with a slow, noisy plant → [[networking/08-congestion-control|Congestion Control]]
...
## Related
- [[control-theory/02-modelling-and-transfer-functions|Modelling and Transfer Functions]] — how to describe a plant
- [[control-theory/04-pid-control|PID Control]] — the controller you'll actually use
