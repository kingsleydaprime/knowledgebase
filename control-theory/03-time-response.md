...
**The geometric reading of pole location** is worth internalising, because it's what makes [[control-theory/05-stability-and-root-locus|root locus]] design intuitive:
...
> **Each integrator in the loop eliminates the steady-state error to one higher order of input.** That is precisely why the **I** term in a PID controller kills steady-state error — it adds an integrator, taking the system from type 0 to type 1. → [[control-theory/04-pid-control|PID Control]]
...
So delay costs you phase margin — the thing keeping you stable — while giving nothing back. → [[control-theory/06-frequency-response|Frequency Response]]

Because $e^{-sT}$ isn't rational, it's often approximated for analysis by a **Padé approximation**:

$$e^{-sT} \approx \frac{1 - sT/2}{1 + sT/2}$$

Note the numerator — that's a **right-half-plane zero**. Delay behaves like a non-minimum-phase element, with all the bandwidth limitations that implies. → [[control-theory/02-modelling-and-transfer-functions|RHP zeros]]
...
- Computation time
- **Sampling** — a zero-order hold adds an average delay of half a sample period → [[control-theory/12-digital-control|Digital Control]]
...
That gives you $G(s) = Ke^{-\theta s}/(\tau s + 1)$ or a second-order fit — enough to design a controller. **This is what Ziegler–Nichols open-loop tuning is doing**, and it's why a step test is the first thing you do to an unfamiliar plant. → [[control-theory/04-pid-control|PID Control]]
...
## Related
- [[control-theory/02-modelling-and-transfer-functions|Modelling and Transfer Functions]] — where poles come from
- [[control-theory/04-pid-control|PID Control]] — using this to tune
- [[control-theory/05-stability-and-root-locus|Stability and Root Locus]] — moving poles deliberately
