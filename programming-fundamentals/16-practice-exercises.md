...
**Work in order.** Several build on earlier output. Solutions in [[programming-fundamentals/17-practice-exercises-solutions|note 16]] — **genuinely try each one first.** The failures are the lesson; a solution you read is a solution you don't own.
...
**Done when:** all three produce a correct result, or you can name exactly which step was ambiguous. **Most people's first attempt fails on the empty list and on all-negatives** — the second because they started with "assume the largest is 0" → [[programming-fundamentals/01-what-a-program-actually-is|note 01]].
...
**Done when:** you can list what genuinely differed (type declarations, statement terminators, block delimiters, printing) and what didn't (the *idea*). This is the claim in [[programming-fundamentals/02-languages-and-the-translation-problem|note 02]] that the second language costs a fraction of the first — verify it rather than believe it.
...
**Done when:** you have four messages written down, and for at least one of them **the reported line is not the line you broke.** Understanding why is the point → [[programming-fundamentals/04-syntax-and-the-shape-of-a-statement|note 04]].
...
**Done when:** you were wrong about at least one, and the whole exercise took under five minutes → [[programming-fundamentals/03-where-code-gets-written|note 03]].
...
**Done when:** you can state the one rule that explains all four results, and you've seen a shallow copy fail on nested data → [[programming-fundamentals/05-variables-and-types|note 05]].
...
**Done when:** the naive version is provably wrong, and your fix uses integer pence or a decimal type — not rounding at the end, which hides the bug rather than removing it → [[programming-fundamentals/05-variables-and-types|note 05]].
...
**Done when:** the second version handles a new rule without touching any conditional logic. **This is the single most useful small exercise in the set** — it's the difference between code that works and code that absorbs change → [[programming-fundamentals/06-control-flow|note 06]] · [[programming-fundamentals/07-collections|note 07]].
...
**Done when:** you have two timings and their ratio, and you can explain the difference from how each structure finds things. [[programming-fundamentals/07-collections|Note 07]] claims this is the most common beginner performance bug — **confirm the number yourself.**
...
**Done when:** every resulting function can be described **without using the word "and"**, and you can test the grade-assignment logic without a file existing → [[programming-fundamentals/08-functions|note 08]] · [[programming-fundamentals/11-planning-before-you-type|note 11]].
...
**Done when:** all five behave sensibly *and you decided deliberately* what `n = 0` and `n > len` should do, rather than accepting whatever fell out → [[programming-fundamentals/07-collections|note 07]].
...
**Done when:** you've seen the actual error, can say what ran out, and can state why the loop version doesn't hit it → [[programming-fundamentals/09-recursion-and-the-call-stack|note 09]].
...
**Done when:** you have three timings, and you can state roughly how many times `fib(25)` was computed in the naive version → [[programming-fundamentals/09-recursion-and-the-call-stack|note 09]].
...
**Done when:** your log has fewer than ~8 entries. If you found it by reading, do it again with a different bug — **the discipline is the exercise**, not the finding → [[programming-fundamentals/10-errors-and-debugging|note 10]].
...
**Done when:** you have the pseudocode, the program, and an honest count. Then do exercise 15 *without* planning and compare the counts → [[programming-fundamentals/11-planning-before-you-type|note 11]].
...
**Done when:** every attempt to corrupt it from outside fails, or you can name exactly which language feature would prevent it → [[programming-fundamentals/13-objects-and-classes|note 13]].
...
**Done when:** both produce identical output, and you can say which you'd rather debug at 3 a.m. and why. There is no correct answer — **having a reason is the exercise** → [[programming-fundamentals/14-programming-paradigms|note 14]].

---

## When you're done

**You are not finished with this course until you've built something nobody told you to build** → [[programming-fundamentals/12-choosing-what-to-build-next|note 12]]. These exercises are reps; a project is the game.

Then: [[dsa/index|DSA]] for efficiency, or [[languages/06-python/index|Python]] to go deep in one language.

## Related
- [[programming-fundamentals/17-practice-exercises-solutions|Solutions]] — after you've tried
