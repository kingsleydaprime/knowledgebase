...
The layer directly beneath [[foundations/networking/index|networking]], [[languages/01-java/02-jvm-and-concurrency/index|the JVM]], [[devops/01-linux/index|Linux]], and every container you'll ever run. Processes, memory, scheduling, and I/O — the machinery your runtime is standing on.
...
- [[foundations/os/interview/index|interview/]] — processes vs threads, page faults, syscall cost, the page cache, scheduling, deadlock, blocking vs async I/O, `LD_PRELOAD`, "everything is a file"
...
- **Kernel networking internals** — the path from NIC to socket, beyond what [[foundations/networking/index|networking]] covers
- **Windows and macOS** — this is Linux-shaped throughout

## Related
- [[foundations/networking/index|Networking]] — the layer above, built on sockets and file descriptors
- [[devops/01-linux/index|Linux]] — the same machine from the command line
- [[foundations/computer-architecture/index|Computer Architecture]] — the layer directly below: the TLB behind virtual memory, cache coherence behind concurrency primitives, and memory models behind atomics
