...
Solutions with measured results in [[mathematics/07-applied-and-computational/01-numerical-methods/12-practice-exercises-solutions|note 12]].
...
**Done when:** you have a clear **V shape**, you can name which error dominates on each arm, and you can predict where the minimum sits from machine epsilon alone — *before* looking at your plot → [[mathematics/07-applied-and-computational/01-numerical-methods/02-floating-point-and-error|note 02]].
...
**Done when:** the naive small root is visibly wrong, you can quantify the relative error, and you can name the mechanism that destroyed it → [[mathematics/07-applied-and-computational/01-numerical-methods/02-floating-point-and-error|note 02]].
...
**Done when:** you can state roughly how many correct digits survive at each $n$, and relate that number to $\log_{10}\kappa(H)$ → [[mathematics/07-applied-and-computational/01-numerical-methods/04-linear-systems|note 04]].
...
**Done when:** you have four different results, and can order them by accuracy *and explain the ordering* → [[mathematics/07-applied-and-computational/01-numerical-methods/02-floating-point-and-error|note 02]].
...
**Done when:** you can state the convergence order of each from your own iteration counts — the error should roughly *square* each step for Newton → [[mathematics/07-applied-and-computational/01-numerical-methods/03-root-finding|note 03]].
...
**Done when:** you have all three failures reproduced, and for (c) you can say what the convergence order dropped to and why → [[mathematics/07-applied-and-computational/01-numerical-methods/03-root-finding|note 03]].
...
**Done when:** the unpivoted version returns a badly wrong answer on a *well-conditioned* system, which is the point — **this is an algorithm failure, not a conditioning failure** → [[mathematics/07-applied-and-computational/01-numerical-methods/04-linear-systems|note 04]].
...
**Done when:** the equally-spaced error *grows* with more points while the Chebyshev error shrinks. **More data made it worse — that's the lesson** → [[mathematics/07-applied-and-computational/01-numerical-methods/06-interpolation-and-approximation|note 06]].
...
**Done when:** the explicit method either explodes or needs orders of magnitude more steps, and you can state the stability condition that forced it → [[mathematics/07-applied-and-computational/01-numerical-methods/08-ordinary-differential-equations|note 08]].
...
**Done when:** you have both counts, and can explain the difference in terms of the Hessian's condition number rather than "BFGS is better" → [[mathematics/07-applied-and-computational/01-numerical-methods/10-numerical-optimisation|note 10]] · [[mathematics/07-applied-and-computational/01-numerical-methods/05-eigenvalues|note 05]].
...
## Related
- [[mathematics/07-applied-and-computational/01-numerical-methods/12-practice-exercises-solutions|Solutions]] — with measured results
- [[mathematics/07-applied-and-computational/01-numerical-methods/index|the course]]
- [[computer-architecture/13-practice-exercises|architecture exercises]] — where the floating point lives
