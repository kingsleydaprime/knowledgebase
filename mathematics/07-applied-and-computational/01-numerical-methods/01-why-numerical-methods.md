...
**Source:** `[reference]` — see [[mathematics/07-applied-and-computational/01-numerical-methods/index|the domain note]].
...
- **Navier–Stokes** — existence and smoothness is an open Millennium Prize problem → [[continuum-mechanics/11-viscous-fluids-and-navier-stokes|Navier–Stokes]]
...
|---|---|
| [[continuum-mechanics/13-computational-methods-and-fem\|FEM]] | quadrature, linear solvers, stability |
...
| [[continuum-mechanics/index\|continuum mechanics]] | **says so explicitly in its gaps section** |
...
**Floating point has finite precision**, so every operation introduces relative error around $10^{-16}$ for a `double`. → [[computer-architecture/02-data-representation|Floating Point]]
...
- **Matrix:** $\kappa(A) = \|A\|\,\|A^{-1}\|$. **$\kappa = 10^k$ means you lose about $k$ digits.** → [[mathematics/07-applied-and-computational/01-numerical-methods/04-linear-systems|Linear Systems]]
...
**Prerequisites:** calculus (Taylor series especially), [[ai-ml/00-foundations/03-mathematics/01-linear-algebra/index|linear algebra]], and [[computer-architecture/02-data-representation|floating point]]. **Taylor series is the tool the whole subject runs on** — nearly every method and every error bound comes from truncating one.

---

## Related
- [[mathematics/07-applied-and-computational/01-numerical-methods/02-floating-point-and-error|Floating Point and Error]] — the foundation
- [[mathematics/07-applied-and-computational/01-numerical-methods/04-linear-systems|Linear Systems]] — the most-used note here
- [[continuum-mechanics/13-computational-methods-and-fem|FEM]] — the domain that asked for this
