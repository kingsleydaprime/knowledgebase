...
| Problem | Reduces to |
|---|---|
| [[mathematics/07-applied-and-computational/01-numerical-methods/03-root-finding\|Newton's method]] | $J\Delta x = -F$ each iteration |
| [[continuum-mechanics/13-computational-methods-and-fem\|FEM]] | $Ku = f$ — often millions of unknowns |
| [[mathematics/07-applied-and-computational/01-numerical-methods/09-partial-differential-equations\|Implicit PDE solvers]] | a linear solve per time step |
...
> **Partial pivoting is why LAPACK's `dgesv` is backward stable in practice and your textbook implementation isn't.** It costs almost nothing — $O(n^2)$ comparisons against $O(n^3)$ arithmetic — and it's not optional. → [[mathematics/07-applied-and-computational/01-numerical-methods/02-floating-point-and-error|Backward stability]]
...
**This is a property of the problem, not the algorithm.** A perfectly stable solver on an ill-conditioned system still gives you a bad answer, **and that's not a bug to fix.** → [[mathematics/07-applied-and-computational/01-numerical-methods/01-why-numerical-methods|Conditioning vs stability]]
...
## Related
- [[mathematics/07-applied-and-computational/01-numerical-methods/05-eigenvalues|Eigenvalues]] — the other big linear-algebra computation
- [[mathematics/07-applied-and-computational/01-numerical-methods/09-partial-differential-equations|PDEs]] — where the huge sparse systems come from
