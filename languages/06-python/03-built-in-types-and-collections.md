...
**Two of those rows are where slow Python comes from.** `insert(0, x)` in a loop is quadratic — use `collections.deque`. And `x in lst` inside a loop over another list is O(n×m) — use a `set` → [[programming-fundamentals/07-collections|collections]].
...
- [[dsa/index|DSA]] — why these costs are what they are
- [[programming-fundamentals/07-collections|collections]] — the language-agnostic version
