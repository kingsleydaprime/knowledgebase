...
**`std::nth_element`** is under-used: it puts the *n*th element where it would be if sorted, in O(n), which is the right tool for medians and top-k. → [[dsa/04-patterns/07-top-k-elements|Top-K Elements]]
