...
**Goroutines leak, and nothing tells you.** A goroutine blocked forever on a channel is never collected. Unbounded spawning — one per request, per file, per loop iteration, with no bound and no exit path — is _the_ characteristic Go bug.
...
- Closing is a _broadcast_: every receiver unblocks. That's why a `chan struct{}` closed as a signal is the idiomatic "stop everyone" primitive.
...
An unbuffered send with no receiver blocks forever. The runtime detects the case where _every_ goroutine is blocked and panics with a useful message — but it cannot detect a partial deadlock where one goroutine hangs while others run. Those show up as a request that never returns.
