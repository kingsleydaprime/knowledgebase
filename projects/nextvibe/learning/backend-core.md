...
`learning/sys-design.md` (architecture + database design), `learning/backend-modules.md` (the
domain module walkthroughs — Events, Games, Billing, Payments, Notifications, Social, Discovery,
Messaging, Storage, Admin, Pledges), `learning/backend-auth.md` (JWT/OAuth/guards),
`learning/backend-realtime.md` (WebSocket gateways and Socket.io), `learning/backend-games-ai.md`
(AI generation + anonymous play), and `learning/devops.md` (deployment/infra).
...
(See `learning/sys-design.md` Part 4 for why this project is a monolith rather than microservices, and the full technology stack rationale.)
...
As mentioned in `learning/backend-realtime.md`, running multiple API instances breaks WebSocket notifications. Add:
...
(See `learning/backend-auth.md` for the full JWT access/refresh token architecture this bug lives inside.)
...
The `notifications` table (which will be queried on every page load for unread count) doesn't have a composite index on `[recipientId, isRead]`. Adding this would make the unread count query significantly faster. (See `learning/sys-design.md` for the full indexing theory this gap relates to.)
...
When a feature "isn't working" and you can't find any error, check for duplicate route registration. It's invisible in logs and easy to miss during refactors where responsibility for a domain shifts from one module to another. (See `learning/devops.md` Part 31 for how to spot this from server startup logs.)
...
(See `learning/backend-modules.md` for the full Notifications module architecture, including the two-layer WebSocket + email delivery pattern, and Part 52 there for the notification coverage audit.)
...
(See `learning/devops.md` Part 35 for exactly how the presigned URL's signature works, and `learning/frontend-uploads-errors.md` for the full frontend implementation, including progress bars and image compression.)
...
(See `learning/backend-modules.md` Part 51 for another instance of this exact rule: `GET /purchases/:id/summary` must be declared before `GET /purchases/:id` in the payments controller.)
...
(See `learning/devops.md` Part 33 for the full process-level signal handling — `uncaughtException`, `SIGTERM`, `SIGUSR2`, and the force-exit timeout pattern — that this hook plugs into.)
