...
See also `learning/frontend-routing.md` (the `src/proxy.ts` middleware this auth system relies
on), `learning/frontend-state-management.md` (the `baseQueryWithReauth` RTK Query base query),
`learning/frontend-realtime.md` (Socket.IO auth handshake, which shares the same cookies), and
`learning/backend-auth.md` for the backend half of this same JWT system (Argon2 hashing, the
...
Without the queue, A, B, and C would each call `/refresh`, rotating the token 3 times and invalidating the first two. (See `learning/backend-auth.md` for the backend side of this exact rotation — the refresh token is deleted from Redis and reissued on every use, which is exactly why a client-side queue is required.)
...
This allows existing admin sessions (that only have `admin_accessToken`) to access non-admin pages without requiring a re-login. (This is the `src/proxy.ts` middleware described in `learning/frontend-routing.md` Part 23.)
...
`"error"` is the sentinel for "no token". The socket never attempts to connect. (See `learning/frontend-realtime.md` for the full `useSocket` hook this snippet belongs to.)
