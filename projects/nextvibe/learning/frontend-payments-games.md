...
See also `learning/frontend-state-management.md` (RTK Query lazy queries used for payment
polling), `learning/frontend-auth.md` (the login flow anonymous play hooks into),
`learning/frontend-realtime.md`, `learning/frontend-performance-debugging.md`, and — for the
backend half of this feature area — `learning/backend-modules.md` (Payments, Pledges, Ercaspay
internals) and `learning/backend-games-ai.md` (AI generation + the anonymous play system this
...
(See `learning/frontend-performance-debugging.md` Part 27 for the general "stale `useEffect` not re-triggering" lesson this bug is an instance of.)
...
(See `learning/backend-modules.md` Part 51 for the backend's Ercaspay internals — NGN vs kobo amounts and the webhook branching this frontend flow ultimately depends on.)
...
(See `learning/backend-games-ai.md` Part 53 for the matching backend-side bug — a config-shape mismatch that silently zeroed every word-puzzle score — and Part 57 below for the wizard-side serialization step that produces the correct config shape.)
...
(See `learning/backend-games-ai.md` Part 48 for two more real dead-code examples found alongside this one — an unused `save-draft` backend endpoint and an entire unused frontend API file, `gameApi.ts`.)
...
(See `learning/backend-games-ai.md` Part 49 for the full backend side of this system — the Redis-backed guest session store and the three endpoints this file's frontend code calls.)
...
Note: this is different from `PUBLIC_ROUTES` in `src/proxy.ts` (server-side middleware, see `learning/frontend-routing.md` Part 23). The `PUBLIC_PATHS` check is purely client-side — it only prevents `baseQueryWithReauth` from redirecting to login on 401.
...
Reading `getAnonymousId()` directly from localStorage breaks out of the closure — localStorage is always current. This pattern (`stateVar ?? readFromSource()`) is the general fix for stale closure problems where the source of truth is outside React. (See `learning/frontend-realtime.md` Part 48 for the general stale-closure concept this pattern is fixing.)
...
No auth header needed. The purchase UUID is the access control: a 128-bit UUID (2^122 possible values) is computationally infeasible to guess. This lets the confirmation page work even if the user isn't logged in at that moment. (See `learning/backend-modules.md` Part 51 for the backend route-ordering detail that makes this endpoint work correctly alongside `GET /purchases/:id`.)
...
(See `learning/frontend-routing.md` Part 25 for why this `<Suspense>` wrapper is a hard Next.js build requirement, not a style choice.)
...
The backend enum stays `THIS_OR_THAT` — no database migration needed. The display label changes to "True or False" and the mechanics change. (See `learning/backend-games-ai.md` Part 53 for the corresponding backend scoring change.)
...
(See `learning/backend-games-ai.md` Part 48 for the OpenRouter/Gemini AI generation service that produces these questions.)
...
(See `learning/backend-games-ai.md` Part 53 for the exact backend config shape this serialization step must match, and the real bug that occurred when an earlier version of this wizard didn't group hidden words correctly.)
