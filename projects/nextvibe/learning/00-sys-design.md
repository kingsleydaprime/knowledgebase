...
`learning.md` and `frontend-learning.md` (moved to `learning/archive/`, for reference).
See also `learning/backend/01-core.md`, `learning/backend/03-modules.md`, `learning/backend/02-auth.md`,
`learning/backend/05-realtime.md`, `learning/backend/04-games-ai.md`, `learning/09-devops.md`, and the
...
**What's missing:** The `notifications` table has no index on `[recipientId, isRead]`. The unread count query is `WHERE recipientId = ? AND isRead = false`. Without this compound index, every page load that shows a notification badge does a full scan of all notifications for that user. At 100 notifications per user and 10,000 users making simultaneous requests, this becomes a problem fast. This is noted in `learning/backend/03-modules.md` (Part 20 — What is Missing).
