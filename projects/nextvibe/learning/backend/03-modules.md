...
`learning/backend/01-core.md` (NestJS/Prisma fundamentals and cross-cutting gotchas),
`learning/backend/02-auth.md`, `learning/backend/05-realtime.md` (WebSocket gateway internals),
`learning/backend/04-games-ai.md` (AI generation + anonymous play — a deeper dive on top of the
Games Module overview below), `learning/00-sys-design.md` (the schema this all sits on), and
`learning/09-devops.md`.
...
The downside: every query must include `deletedAt: null` or you'll accidentally show deleted records. This is easy to forget and is a common source of bugs. (See `learning/backend/01-core.md` Part 20 — soft delete is not applied consistently across the domain.)

---

## Part 10 — The Games Module (Most Complex Logic)

The games module has the most business logic in the codebase. Understanding it teaches you how to model complex state machines. (For the AI generation side of Games and the anonymous/guest play system, see `learning/backend/04-games-ai.md`.)
...
(See `learning/backend/04-games-ai.md` Part 53 and Part 56 for later changes to this scoring logic — a serialization bug in `WORD_PUZZLE` config, and the pivot of `THIS_OR_THAT` from a participation game to a real True/False knowledge game.)
...
**Scaling problem:** Socket.io rooms are in-memory in this process. If you run two API instances, a user connected to instance 1 won't receive events emitted by instance 2. The fix: Socket.io Redis adapter (the `redis.service.ts` even exposes `getClient()` for this exact reason). This is one of the first things to add when you need horizontal scaling. (See `learning/backend/01-core.md` Part 20 for this same gap in the wider "what's missing" list.)

(For the full generic WebSocket gateway authentication pattern — verifying the JWT, not just checking presence — see `learning/backend/05-realtime.md` Part 26.)
...
(See `learning/backend/01-core.md` Part 28 for the full convention on what `targetId` should point to for each notification type — including a real bug where `FOLLOW` notifications had `targetId` set to the actor instead of the target.)
...
(Note: `learning/backend/01-core.md` Part 30 covers a real HTTP-method mismatch bug on the `:id/read` endpoint — the controller used `@Post` while some clients expected `PATCH`, producing a misleading 404.)
...
(For the general WebSocket gateway auth pattern that both `/messaging` and `/notifications` rely on, see `learning/backend/05-realtime.md` Part 26. For the frontend side of DMs and event chat — optimistic messages, chat UI bubbles, real-time badges — see `learning/frontend/06-realtime.md`.)
...
(See `learning/backend/01-core.md` Part 32 for how this module's upload flow was migrated from multipart uploads through NestJS to presigned URLs, and `learning/09-devops.md` Part 34/35 for the MinIO configuration and presigned URL signature mechanics behind it.)
...
(See `learning/backend/02-auth.md` Part 46 for the `@Public()` + optional-user pattern this endpoint relies on.)
...
**Route ordering is critical:** `GET /purchases/:id/summary` must be declared BEFORE `GET /purchases/:id` in the controller. Otherwise NestJS matches the literal string `"summary"` as the `:id` param and tries to parse it as a UUID, producing a 400. This is a specific instance of the static-before-parameterized rule from `learning/backend/01-core.md` Part 42.

(See `learning/frontend/07-payments-games.md` Part 53 for the frontend confirmation page that consumes this endpoint, including the webhook-delay polling pattern.)
...
Note: anonymous players (`entry.userId === null`) don't get notifications — they have no account to receive them. The `if (entry.userId)` guard handles this. (See `learning/backend/04-games-ai.md` Part 49 for the full anonymous game play system this guard interacts with.)
