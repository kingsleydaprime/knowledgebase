...
`learning/backend-core.md` (NestJS/Prisma fundamentals), `learning/backend-modules.md` (domain
modules), `learning/backend-realtime.md` (WebSocket gateway authentication specifically),
`learning/sys-design.md`, `learning/devops.md`, `learning/backend-games-ai.md`, and the
`learning/frontend-auth.md` file for the frontend side of this same system (token storage,
...
**The current bug:** `expiresIn` is commented out in `generateTokens()`. Access tokens never expire. This eliminates the short-damage-window benefit entirely. Fix this before launch by uncommenting and setting `expiresIn: '15m'` for access tokens. (This is tracked as a P0 security gap in `learning/backend-core.md` Part 20 — What is Missing.)
...
(See `learning/frontend-auth.md` Part 37 and Part 43 for two real production bugs in the frontend's Google login button — a wrong cookie expiry that silently killed WebSocket auth, and a `?from=` redirect encoding bug.)
...
(See `learning/backend-modules.md` Part 50/51 for the pledges and Ercaspay payment modules, which use this same `@Public()` + optional-user pattern to support both guest and authenticated flows on the same endpoint. See `learning/backend-games-ai.md` Part 49 for the anonymous game play system, another major consumer of "public route, optional identity" patterns.)
