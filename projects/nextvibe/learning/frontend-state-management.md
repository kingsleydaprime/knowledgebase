...
See also `learning/frontend-routing.md` (Server vs Client Components — the context this state
layer lives inside), `learning/frontend-auth.md` (the `baseQueryWithReauth` token-refresh queue),
`learning/frontend-realtime.md` (how Socket.IO events feed back into the RTK Query cache),
`learning/frontend-performance-debugging.md`, and the other `learning/frontend-*.md` files.
...
`.unwrap()` throws on error instead of returning it — lets you use `try/catch`. (See `learning/frontend-uploads-errors.md` for the universal `errorHandler()` function most `catch` blocks in this project actually call.)
...
Used in the payment verify page to poll manually instead of fetching on mount. (See `learning/frontend-payments-games.md` Part 13 for the full payment polling flow this lazy query is part of.)
...
(See `learning/frontend-auth.md` for the full mechanics of this refresh queue, plus the `PUBLIC_PATHS` allowlist that prevents it from redirecting anonymous users to login on public pages.)
...
Never put secrets in `NEXT_PUBLIC_` variables — they'll appear in the compiled JavaScript. (See `learning/devops.md` Part 38 for the equivalent backend-side environment variable dev/prod split rule.)
...
When a user's network drops and reconnects, Socket.IO auto-reconnects the socket. But **room membership is not preserved** — you must re-join. Always put your `socket.emit("join:*", ...)` inside the effect that depends on `isConnected`, so it fires again after every reconnection. (See `learning/frontend-realtime.md` for the full, corrected event-driven version of this pattern — the naive `isConnected`-dependent effect described here has a real race condition that Part 36 there walks through and fixes.)
