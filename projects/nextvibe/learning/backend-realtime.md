...
`learning/backend-core.md`, `learning/backend-modules.md` (Part 13 — Notifications Module, Part
16 — Messaging Module, which this file's gateway patterns serve), `learning/backend-auth.md`,
`learning/sys-design.md`, `learning/devops.md`, and — for the frontend half of this same
real-time system (the `useSocket` hook, event-driven join pattern, chat UI, notification badges) —
`learning/frontend-realtime.md`.
...
If User B never emits `join:dm`, they are connected but not in any room. Step 6 never reaches them. They'd have to poll the REST API to see new messages. **This is exactly why messages weren't real-time** — the frontend was connecting but not emitting `join:dm`. (See `learning/frontend-realtime.md` Part 36 for the exact frontend race condition — `isConnected` React state vs socket.io's own `"connect"` event — that caused this.)
...
(This is the earlier, simpler version of the hook. See `learning/frontend-realtime.md` for the production `useSocket` hook this evolved into, and the event-driven join fix for the race condition described above.)
