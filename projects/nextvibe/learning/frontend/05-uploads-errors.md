...
See also `learning/frontend/04-forms-ui.md` (the zod file-validation patterns that gate these
uploads), `learning/frontend/02-state-management.md` (RTK Query mutations these upload flows call
into), `learning/backend/01-core.md` Part 32 (the backend side of the multipart→presigned-URL
migration), and `learning/09-devops.md` Part 34/35 (MinIO configuration and how the presigned URL
...
The backend no longer needs `FileFieldsInterceptor` — it just receives a JSON object. (See `learning/backend/01-core.md` Part 32 for the full backend-side migration this replaced, including why `flierUrl` had to become optional in the DTO.)
...
**No video compression client-side:** `browser-image-compression` only handles images. Video compression in the browser is too slow (minutes for large files). Users must pre-compress or stay within the size limit. (See `learning/09-devops.md` Part 34 for the server-side short-term fix — this same compression library — and the longer-term Cloudflare R2 migration recommended alongside it.)
