...
See also `learning/frontend/03-auth.md` (the cookie/token plumbing this system's `auth: { token }`
handshake depends on), `learning/frontend/02-state-management.md` (RTK Query cache that the socket
handlers refetch into), `learning/frontend/01-routing.md`, `learning/frontend/04-forms-ui.md`, and —
for the backend half of this exact real-time system (gateway JWT verification, namespaces,
rooms, the Messaging and Notifications modules) — `learning/backend/05-realtime.md` and
`learning/backend/03-modules.md`.
...
- Auth is via `auth: { token }` in the handshake — this is what the Socket.IO server reads. Query params (the old approach `?token=...`) are less secure and non-standard. (See `learning/frontend/03-auth.md` Part 37 for a real bug where a wrong cookie expiry silently broke this token read.)
...
(See `learning/backend/05-realtime.md` Part 39 for the server-side confirmation of why this bug was invisible from the backend's perspective — the room-join call simply never reached it.)
...
**Rule**: use `setHideHeader` when you control the navigation chrome. Use `fixed inset-0 z-[...]` only for components that render outside the normal layout tree (e.g. portals, dialogs). (See `learning/frontend/02-state-management.md` for the `ui` Redux slice this `hideHeader` flag lives in.)
...
In React, every render creates new function instances. If your effect uses a function from the current render, the deps array tells React when to create a fresh one. Missing a dep = stale closure = bugs that are hard to track down because the code looks correct. (See `learning/frontend/07-payments-games.md` Part 51 for another real stale-closure bug in this codebase — the anonymous game submit handler reading React state instead of localStorage directly.)
