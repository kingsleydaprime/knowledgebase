...
See also `learning/frontend/01-routing.md` (Server/Client Component boundaries, which most of the
performance advice here is about), `learning/frontend/02-state-management.md` (RTK Query caching
knobs), `learning/frontend/06-realtime.md` (Socket.IO listener cleanup), and
`learning/frontend/07-payments-games.md` Part 13 for a real `useEffect` re-trigger bug this file's
...
(See `learning/frontend/01-routing.md` Part 35 for the full Suspense deep dive.)
...
(See `learning/frontend/02-state-management.md` for the full Redux Toolkit slice pattern this rule applies to.)
...
(See `learning/frontend/06-realtime.md` for the corrected, event-driven version of socket effects generally — the `isConnected` dependency shown here is actually a known source of a race condition, not just a cleanup risk.)
...
Webhook routes are called by Ercaspay's servers, not by your users. They don't have your JWT. Don't put them behind `baseQueryWithReauth`. They're public routes secured by HMAC signature verification on the backend. (See `learning/backend/03-modules.md` Part 12 for the backend's HMAC webhook verification this rule protects.)
...
(See `learning/frontend/03-auth.md` Part 33 and Part 43 for the full history of bugs found in this exact flow.)
...
**Lesson**: If a `useEffect` isn't re-running when you expect it to, the issue is almost always its dependency array. Add a counter state (`retryKey`, `refreshKey`, `key`) to its deps when you need to force a re-run without changing the actual data. (See `learning/frontend/07-payments-games.md` Part 13 for the real payment-verification page this exact bug and fix came from.)
