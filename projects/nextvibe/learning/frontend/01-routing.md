...
See also `learning/frontend/02-state-management.md`, `learning/frontend/03-auth.md`,
`learning/frontend/06-realtime.md`, `learning/frontend/04-forms-ui.md`,
`learning/frontend/05-uploads-errors.md`, `learning/frontend/07-payments-games.md`, and
`learning/frontend/08-performance-debugging.md` for the rest of the frontend material, and the
`learning/backend/*.md` / `learning/00-sys-design.md` / `learning/09-devops.md` files for the backend
...
An alternative to RTK Query. Better suited when you don't need Redux for local state — lighter and simpler API. RTK Query is the right choice when you already have Redux in the project (like here). (See `learning/frontend/02-state-management.md` for the RTK Query patterns actually used in this project.)
...
(See `learning/frontend/08-performance-debugging.md` Part 27 for a real debugging incident that hinged on recognising `proxy.ts` as live middleware despite its unusual name.)
...
This is more efficient than the client-side queue pattern — the page receives a fresh token in its very first request and never gets a 401 at all. (See `learning/frontend/03-auth.md` for the client-side `baseQueryWithReauth` queue pattern this middleware refresh complements.)
...
The login page reads `searchParams.get("from")` and calls `router.push(from)` after a successful login. (See `learning/frontend/03-auth.md` Part 33 and Part 43 for the full set of bugs found in this exact `?from=` flow — an un-decoded value, a bad default, and a redirect loop guard.)
...
(See section 35 below for a full deep dive on `<Suspense>` itself, and `learning/frontend/07-payments-games.md` Part 53 for another real page — the ticket purchase confirmation page — that needed this exact wrapper.)
