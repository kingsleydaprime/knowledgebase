...
**~11,000 words across 5 notes.** `[reference]`. Assumes [[languages/05-cpp/index|the C++ course]].
...
**C++ on the server is defensible far more often than [[backend/frameworks/c/index|C]]** — RAII and containers remove the easy memory bugs while keeping the performance. It is still not memory-safe.

**For a new standalone service with no existing C++ and no C++-only libraries, [[backend/frameworks/rust/index|Rust]] is the better choice** — same performance, guarantees instead of discipline, and a much better web ecosystem. Or [[backend/frameworks/go/index|Go]], which wins decisively on everything except raw performance and footprint.
