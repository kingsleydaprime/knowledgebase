...
Ethereum uses a variant, the **Merkle Patricia Trie**, which is also a *map* — it proves "account X has balance Y" and, crucially, proves **absence** ("this key is not in the state"), which a plain Merkle tree can't do → [[dsa/02-data-structures/05-trees/01-trees|trees]].
