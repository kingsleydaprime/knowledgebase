...
## Current priority — teaching quality, not more coverage (2026-09-05)
...
- [x] Establish one shared teaching standard; align the root README and [[learning/COURSE-TEMPLATE|personal course template]] with it.
- [x] Revise [[languages/06-python/06-iterators-generators-and-comprehensions|Python iterators and generators]] — suspended-state trace, runnable pipeline, independent lazy-processing task, checks and hints.
- [x] Revise [[databases/03-storage-and-page-layout|storage and page layout]] — exact byte offsets, runnable page model, independent compaction, and capacity/stable-slot checks.
...
2. **Finish one pilot course at a time:** revise Python or databases in prerequisite order, attaching existing exercises to the appropriate lessons and a cumulative artifact to the course index. Do not rewrite every domain simultaneously.
3. **Apply the standard across other domains:** sample each course first; adapt runnable labs to proofs, designs, or hardware experiments as appropriate. Audit course progression as well as individual notes.
4. **Review the DSA exemplars too:** keep their teaching strengths, but verify language-specific claims and add independent implementation where self-assessment currently stops at recall.

**Batch acceptance:** preserve paths and useful depth; verify new links; execute runnable examples and check exercise contracts; label anything untested; then obtain feedback from an actual study attempt before extending the approach. No word-count target and no vault-wide claim of completion.
...
Following the [[languages/index|rule]]: frameworks live here, languages live there.
...
**The point of this folder:** language-agnostic build guides. Each file is _"here is everything you'll meet building this, and the order to build it in"_ — not a tutorial in one language. Where a step needs a library, suggest options **per language**.
...
3. **The build order** — numbered milestones, each independently testable, each producing something that _runs_
...
- [x] **`01-http-server.md`** ✅ — the one to write first, since it's the most tractable. Milestones: raw socket accept loop → request-line parser → header parser → response writer → routing → static files → keep-alive → concurrency → chunked encoding. _Requested example: "build the parser first, then a route."_
...
- [x] **`08-your-own-container.md`** ✅ — namespaces → cgroups → chroot/pivot_root → a layered filesystem. Ties [[devops/02-docker/index|Docker]] to what the kernel actually does
...
- [x] **`databases/` → a course** — ✅ **DONE** — 12 notes, ~17,900 words, plus a README. **Additive**: all four reference files kept (≈40 inbound links) and positioned as the syntax layer; the course is the _internals_ layer they lacked — pages, B-trees, LSM, the query pipeline, MVCC, WAL, replication, operations

~~Deferred, honestly: computer graphics, information theory, PL theory.~~ **Un-deferred 2026-08-16** — and a re-audit of every README's stated-gaps section found two _more_ load-bearing than any of the three:
...
- [x] **`foundations/gpu-and-parallel-computing/`** — ✅ **DONE** — 7 notes, ~10,800 words. Closes the gap flagged in `computer-architecture/index` ("a genuine gap given the ML material in this vault"). **`ai-ml/` is 98 notes with nothing explaining the hardware underneath.** SIMT/warps → coalescing → occupancy → the CUDA model → why matmul maps well → tensor cores → transfer cost
...
- [x] **`foundations/programming-fundamentals/`** — ✅ **DONE** — README + 12 notes, ~13,700 words. **The vault had no way in.** 900+ notes assumed you could already read code; `using-ai/` was written as the on-ramp for non-programmers into _AI_, and there was no equivalent into _programming_. Language-agnostic: what a program is → translation → tooling → syntax → variables/types → control flow → collections → functions → recursion/the call stack → errors/debugging → planning → what to build next
...
- [x] **`foundations/programming-fundamentals/` notes 13–14** — ✅ **DONE** — ~2,200 words. **Closes the gap this course's own README declared.** The original call — "OOP belongs in `languages/`" — was wrong: OOP is language-_flavoured_, not language-specific, and omitting it stopped the course one concept short of where every real codebase starts. Objects and classes; programming paradigms (with expression-vs-statement, which was absent vault-wide outside `compilers/`)
- [x] **`languages/06-python/`** — ✅ **DONE** — README + 14 notes, ~12,600 words. **`languages/index` had listed "Python at depth" as a track that would slot in if notes got written.** They hadn't been — despite the entire `ai-ml/` domain being written in Python and `devops/` assuming it for automation. **The vault taught Python-the-tool everywhere and Python-the-language nowhere.** Deliberately excludes the web frameworks (below) and the numeric stack (`ai-ml/00-foundations/04-python-and-data-tools/`), per the `languages/` rule
...
- [x] **`languages/06-python/` 18–19** — ✅ ~3,500 words. Eighteen exercises that _reproduce_ the gotchas. **Every figure in the solutions was measured on Python 3.14**, not estimated — including one exercise that had to be rewritten because 3.14's constant folding defeated the original demonstration
...
- **gzip beat the order-0 entropy "floor" by ~2×** on a real file, which forced the correct statement: entropy is the floor _for a given source model_
- **A SQLite index gave 9,327×** on 200 lookups over 500k rows
- **`foundations/information-theory/index` claimed Huffman loses "~7× at $p=0.9$" — measured, that's 2.1×; 7.1× is $p=0.98$.** Corrected in place
...
- [x] **`backend/frameworks/csharp/`** — ✅ README + 4 notes, ~3,500 words. **Named in `languages/07-csharp/index`'s own "what's missing"**, and the frameworks map listed seven languages with C# conspicuously absent right after the language course landed. **Flat notes by concern**, per the folder's own folder-vs-flat test: ASP.NET Core is _the_ choice, and Minimal APIs vs MVC are two styles within it. Also updated the cross-stack concept table to seven columns
- [x] **`build-your-own-shit/` 10–12** — ✅ ~2,900 words. Chosen against the folder's stated criterion rather than to pad the list:
  ...
  - **Physics engine** — completes the game-dev track, and it's the rare project where a wrong integrator is _visually_ wrong
...
- [x] **`backend/06-cross-cutting/`** — ✅ README + 7 notes, ~5,400 words. **This was the actual scaffold** — a README that mapped elsewhere, including a row reading _"file uploads — not covered anywhere yet — a genuine gap."_ Validation & DTOs · config & secrets · error handling · rate limiting · idempotency & retries · security headers & CORS · file uploads. **Caching, observability and background jobs deliberately still point elsewhere** rather than being duplicated
- [x] **`backend/frameworks/cross-language-recipes`** — ✅ ~1,500 words. The stated ask: _"a full backend in all the languages — rate limiting, headers, auth."_ Middleware, rate limiting, JWT verification (**with algorithm pinning**), CORS, **graceful shutdown**, structured logging, and a what-you-get-free matrix — **side by side in Node, Go, Rust, Python, C# and Java**

**Phase 5i total: 9 files, ~7,400 words**, plus three corrected indexes.

### Phase 5j — the frontend restructure (2026-08-23)

**`frontend/` is now a course + `frameworks/`, mirroring `backend/`.** Both moves were already prescribed by the vault: `concepts/index` said _"02-frontend arguably belongs in frontend/. Not moved yet"_, and `frontend/index` said _"distil the recurring material and leave the war stories in projects/, cross-linked — the same split backend uses."_
...
- [x] **`projects/` material left in place, deliberately** — ~44,000 words of React/Next taught against real code. Moving it would strip the project logs of material that reads as a whole, against the [[learning/index|dev-workflow]] principle that both copies are meant to exist. **Indexed from `frameworks/react/` and `frameworks/next/` instead** — the problem was findability, not location
...
- [x] **"Software Engineering — Orientation" → "Introduction to Software Engineering"** — ✅ the domain README title, the root README entry, and **SWE 101's week 1** in the scheme of work. _"Orientation"_ described the note's role in the vault; _"Introduction to Software Engineering"_ describes what it teaches, which is the right name for the **first topic of the course**

**Phase 5k total: 1 new guide + 2 renames across 45 files.**

---

### Phase 6 — the web3 track (2026-08-31)

**His observation, and it was correct:** the vault had **nothing** on blockchain — no smart contracts, no Solidity, no crypto, no NFTs, no chain development in any language. A grep for `web3|blockchain|solidity|smart contract|ethereum|bitcoin` across every note returned six incidental mentions, all of them analogies in `dsa/`, `theory-of-computation/` and `architecture/`.

- [x] **`web3/`** — ✅ **71 files.** An 8-section numbered course, `frameworks/` for five languages, and an `interview/` bank. Built to the `cybersecurity/` shape: `01-foundations` (the double-spend problem as an _ordering_ problem, UTXO vs accounts, consensus read through _what does an attack cost_) → `02-ethereum-and-the-evm` (the four data locations, and the storage-cost ratio that drives everything downstream) → `03-smart-contracts-with-solidity` (10 notes) → `04-smart-contract-security` (8 notes, **and its README says to read it in parallel with 03, not after**) → `05-beyond-ethereum` (the trilemma, rollups, zk, Solana, Bitcoin, bridges) → `06-building-dapps` (**the note that saves the most rework is 04: you cannot query a blockchain**) → `07-the-application-layer` → `08-the-honest-assessment`
- [x] **`build-your-own-shit/14`, `15`, `16`** — ✅ his call to make it three guides rather than one. **14 blockchain** (the payoff is watching two of your own nodes fork and reconverge), **15 smart contract VM** (the deepest — gas, reverts and `DELEGATECALL` become mechanisms you built), **16 token and wallet** (the most applied, and the best first one: an ERC-20 on a testnet in MetaMask, and an HD wallet whose addresses match MetaMask's for the same seed)
- [x] **Integration** — ✅ root README structure tree, `INTERVIEW.md` (fifteen → sixteen banks), `build-your-own-shit/index` (thirteen → sixteen), and two Quartz symlinks
- [x] **A dangling symlink, found in passing** — ✅ `quartz/content/build-your-own-x` still pointed at the pre-5k name and had been broken since the rename, so **the folder was absent from the published site**. Repointed to `build-your-own-shit`

**Section 08 is the part that makes this track worth having.** Every other web3 resource explains how things work; that section asks whether they should be used — the four-question test that kills most blockchain proposals, **where the money actually went** (access control and bridges, not the reentrancy the field talks about), regulation as the constraint that has shaped the field more than any technical limit, and energy after the Merge. The course README says to read it _early_, right after foundations, rather than last.

**The standing caveat applies in full:** this is `[reference]`. Nothing here has been deployed, audited, or exploited by its author. The three build guides exist to start closing that, and guide 16 is the cheapest — a free testnet, one evening, and a contract that exists.

**Phase 6 total: 74 new files, plus 4 indexes updated and 2 symlinks fixed.**

---

### Phase 7 — the reps pass (2026-08-31)

**His call, and the right one:** the vault had ~1M words of map and one 216-line file of territory. _"I want my courses to make me really good"_ — which is a reps problem, not a notes problem.

- [x] **A `projects.md` in every course domain** — ✅ **15 files.** Eight written from scratch for domains that had **nothing** (`backend`, `frontend`, `databases`, `architecture`, `web3`, `git`, `languages`, `engineering`); seven migrated out of the central file and expanded (`devops`, `ai-ml`, `cybersecurity`, `hardware`, `robotics`, `game-development`, `foundations`)
- [x] **A _done when_ on every project** — ✅ the convention borrowed from `build-your-own-shit`'s "works when". **A project without a finish line gets abandoned or gold-plated**, and the old list had neither
- [x] **`project-ideas.md` rewritten as an index** — ✅ 216 → 94 lines. A one-per-domain table naming _the one to do first_, the "if you only do five", and the two sections that are career-target rather than domain-scoped (Java/systems, build-your-own)
- [x] **Linked from every domain README** — ✅ 14 of 15; `foundations/` has no README to link from (see below)

---

## Queued — the gap audit (2026-08-31)

**Found by auditing coverage rather than by asking what's missing.** In priority order.

### Missing domains

- [ ] **`mobile/`** — **the biggest structural hole.** No Swift, Kotlin, Dart, iOS, Android, Flutter or React Native anywhere; every grep hit is incidental. `frontend/`, `backend/`, `desktop/` and `game-development/` all exist — mobile is the missing sibling, and it's the platform most people actually use. Needs `languages/08-swift`, `09-kotlin`, `10-dart` plus `mobile/` with `frameworks/{ios,android,flutter,react-native}` and `interview/`. **Note: Android is Kotlin, not Java** — Google went Kotlin-first in 2019
- [ ] **Data engineering** — Kafka, Spark, Airflow, dbt, warehouses, streaming, CDC. `databases/` is OLTP internals only. The missing bridge between `databases/` and `ai-ml/`
- [ ] **DSP / signal processing** — Fourier, filters, sampling, modulation. The missing link between `hardware/`'s RF material and `foundations/information-theory/`; would also serve `robotics/`
- [ ] XR, quantum computing, search/IR, product & design — real but narrower

### Note gaps inside `cybersecurity/`

**34,822 words total — smaller than the web3 track, for a much bigger subject.** `shellcode`, `kerberos`, `pivoting`, `mimikatz` and `fuzzing` return **zero** files.

- [ ] **Binary exploitation** — currently one bullet and one heading. No shellcode, ROP, heap, ASLR/canary bypass. **This is what OSCP and most CTFs test**
- [ ] **Active Directory / Windows attack paths** — currently **one table row** (Kerberoasting). No Kerberos, pass-the-hash depth, lateral movement, BloodHound. **AD is what the large majority of real corporate pentests are**
- [ ] **`10-protecting-yourself/`** — personal security for a non-technical reader, in the shape `using-ai/` was written for. Password managers, passkeys, SIM-swap, device encryption, backups, a personal threat model, what to do in the first hour after a breach. **The one thing in the vault you could send to family** — and 90% of real compromise is what it would cover
- [ ] **Malware analysis & reverse engineering** — 2 passing mentions
- [ ] **API security** — no dedicated coverage
- [ ] Thicken `09-cloud-security` (**758 words, thinnest folder in the vault**), `08-governance-risk-and-compliance` (1,225), `01-fundamentals` (1,540)

### Structural

- [ ] **`foundations/` has no README** — the vault's largest section (~312,000 words, 16 sub-courses) has no entry point. Every other domain has one. **Found while linking `projects.md` files in Phase 7**
- [ ] **Practice exercises** exist in 12 domains, absent in 10 (backend, frontend, architecture, git, web3, engineering, robotics, hardware, game-dev, concepts). Lower priority than the above — `projects.md` now covers the reps gap for those

**Phase 7 total: 15 new files, 1 rewritten index, 14 READMEs updated.**

---

### Phase 8 — protecting-yourself, then mobile (2026-08-31)

**Both from his ask, in his order.**

- [x] **`cybersecurity/10-protecting-yourself/`** — ✅ **9 files, ~8,600 words.** The odd one out in that domain and deliberately so: sections 01–09 are written for someone defending an organisation, this one for **the reader themselves and the people who ring them.** Written in the [[using-ai/index|using-ai]] shape — no code, no jargon, honest about what's oversold. Your actual threat model (**you are almost certainly not being targeted**) → passwords, where **reuse not strength is the problem** → why SMS 2FA is the weak kind → the phishing tells that still work now AI has killed the old two → devices, and **an honest VPN answer** → backups, and **why sync is not backup** → your privacy footprint as an attacker's research budget → **an ordered checklist for when it's already happened**
- [x] **`mobile/`** — ✅ **23 files, ~18,000 words.** The gap Phase 6's audit named first. A 14-note **platform-agnostic** course (that's the point — it holds whichever stack you pick), `frameworks/{ios,android,flutter,react-native}`, `interview/`, `projects.md`
- [x] **`languages/08-swift/`, `09-kotlin/`, `10-dart/`** — ✅ **29 files, ~19,600 words.** Kotlin is deliberately the shortest of the three because [[languages/01-java/index|the Java course]] already covers the JVM underneath it; Dart is shortest overall because it holds few surprises for anyone who knows Java or TypeScript
- [x] **A correction carried into the build** — ✅ he said the mobile languages were "swift, dart, java, js". **Android is Kotlin, not Java** — Google went Kotlin-first in 2019 — so the track is Swift/Kotlin/Dart, with TypeScript already covered by [[frontend/frameworks/react/index|React]]
- [x] **Integration** — ✅ `languages/index` (the stale _"Kotlin would slot in if notes get written"_ line is now redundant and removed), root README tree, `INTERVIEW.md` (sixteen → seventeen), `project-ideas.md`, the Quartz landing page (six → **seven columns**), and a `mobile` symlink

**The load-bearing note in the mobile track is 03, the lifecycle.** Everything else — view models, saved-state handles, offline-first databases, resumable navigation — is a response to _the OS can destroy your process at any moment with no reliable warning._ Its testing section is the part most developers never do, and the interview bank says plainly that the process-death questions are what separate people who've shipped from people who've followed tutorials.

**And the honest labelling holds:** both tracks are `[reference]`. Nothing in `mobile/` has been shipped to a store by its author, and `mobile/projects.md` says so in its first line. The cheapest fix is its first rung — _survive process death_ — which is an afternoon.

**Phase 8 total: 61 new files, ~46,200 words, plus 6 indexes updated and 1 symlink added.**
...
- [x] **Integration** — ✅ `cybersecurity/index` (two new sections), root README tree, and `cybersecurity/projects.md` — **whose stale "expect to learn these elsewhere until those notes exist" caveat is now three real reps** pointing at GOAD, ROP Emporium and pwn.college
- [x] **A self-inflicted cleanup** — ✅ `ln -s` into the `quartz/content/cybersecurity` dir-symlink created two recursive symlinks inside the real folders; removed. (cybersecurity is a _directory_ symlink, so its subdirs are auto-exposed to Quartz — no per-section symlink needed)
...
- [x] **`data-engineering/`** — ✅ **10 notes + README + projects + interview.** The gap the audit named: `databases/` is single-DB internals, `architecture/04` has the log/partitioning _theory_, `ai-ml/10-mlops` touches serving — but the discipline that _assembles_ pipelines had no home
...
- [x] **Framing that shaped it:** the two load-bearing ideas are **the frequency domain** (any signal is a sum of sinusoids) and **convolution** (which _is_ multiplication in that domain) — the notes are ordered so 01–05 build to those, and 06–09 apply them. The **convolution theorem** (note 05) is the hinge, and it's deliberately connected to CNNs — _a conv layer is DSP convolution with learned kernels_ — which makes this folder upstream of computer vision, not just radio
...
- [x] **Cross-linked** into `information-theory` (applied sibling), `hardware/06-radio-frequency` (the analog side of modulation), `ai-ml/computer-vision` (convolution), plus foundations/index (theory spine + reference list), foundations/projects, and the root README tree. **No Quartz symlink needed** — `foundations/` is a directory symlink, so the new folder is auto-exposed
...
### Phase 12 — malware analysis & reverse engineering (2026-09-01)

**From the queue.** Built as `cybersecurity/13-...`, the **defensive twin of [[cybersecurity/11-binary-exploitation/index|binary exploitation]]** — understanding programs rather than breaking them — completing the offensive-technical trio (11 exploitation, 12 AD, 13 RE/malware).

- [x] **`cybersecurity/13-malware-and-reverse-engineering/`** — ✅ **6 notes + README.** RE (static vs dynamic, formats, the legal line) → static analysis (**the cheap tells — hash/strings/imports — before reading code**) → dynamic analysis (**the safe lab, unpacking in memory**) → how malware works (one lifecycle, recurring techniques) → evasion (**why signature detection is losing, and defence moved to behaviour**) → the workflow (triage-to-deep-dive, IOCs, YARA)
- [x] **Framing:** deliberately _defensive and research_ — malware analysis to build detection, CTF `rev`, vulnerability research, interoperability. The dual-use line is stated in every note; **isolation is framed as a safety requirement** (you run live hostile code), and the practice ladder starts at zero-infection-risk crackmes/CTF before live samples. No "how to write malware" content — the focus is understanding, analysing, detecting
- [x] **No new interview folder** — consistent with 11 and 12; cybersecurity has one bank at `interview/`
- [x] **Integration** — cybersecurity/index (new section), root README tree, `cybersecurity/projects.md` (two reps: a crackme, then a lab-isolated sample + YARA rule). No Quartz symlink (cybersecurity is a directory symlink)

**Phase 12 total: 7 new files, ~9,700 words, plus 3 indexes updated.**

### Phase 13 — API security (2026-09-01)

**From the queue.** Built as `cybersecurity/14-...`, positioned as _"web security minus the browser"_ and structured around the **OWASP API Security Top 10 (2023)** as its backbone.

- [x] **`cybersecurity/14-api-security/`** — ✅ **6 notes + README.** What makes APIs different (**authorization dominates, not injection** — 3 of OWASP's top 5) → authentication (API keys, **the JWT pitfalls checklist**, OAuth, token storage) → **BOLA/IDOR** and the authZ family (BFLA, mass assignment, excessive exposure) → input validation, injection on an API surface, and **SSRF** (Capital One) → rate limiting and **business-flow abuse a rate limit can't catch** → the lifecycle (shadow/zombie APIs, misconfiguration, unsafe consumption, gateways, testing)
- [x] **Deliberately non-duplicative** — it overlaps [[cybersecurity/04-web-security/index|web-security]] and [[backend/05-auth/index|backend/auth]], so it covers only what's _API-specific_ and cross-links the general treatment for injection, OAuth and TLS. Positioned as the attacking-and-defending twin of [[backend/02-api-design/index|API design]]
- [x] **Integration** — cybersecurity/index (new section), root README tree, `cybersecurity/projects.md` (a BOLA/BFLA rep on crAPI), and cross-links added _into_ web-security, backend/api-design and backend/auth (so the building-side notes point at the security-side). No Quartz symlink (cybersecurity is a directory symlink)

**Phase 13 total: 7 new files, ~9,500 words, plus 6 files updated (3 indexes + 3 cross-links).**

### Phase 14 — thicken cloud security (2026-09-01)

**From the queue — the last substantive gap.** `09-cloud-security` was the vault's thinnest folder (one 637-word note for a whole domain).

- [x] **`cybersecurity/09-cloud-security/` — 1 note → 5.** The old single note was genuinely good but undersold the topic, so I split-and-deepened rather than rewrote: shared responsibility model → **identity-is-the-perimeter** (IAM depth, least privilege, and the _privilege-escalation paths_ over-permissioning enables — the same "permissions that reach permissions" idea as API BOLA) → **the attack surface** ranked (public buckets #1, then the metadata-service-via-SSRF path, tied explicitly to Capital One and my new API-security SSRF note) → **container/k8s** (the three layers, every default insecure) → **cloud-native defence** (shift-left, CSPM/CIEM/CNAPP, CloudTrail→SIEM, DevSecOps)
- [x] **Migration handled cleanly** — old note removed, its content preserved and expanded across the five, and the **2 external inbound links** (from `devops/12-sre.../04-devsecops` and `devops/00-physical.../02-virtualisation`) repointed to the new note 01
- [x] **Cross-linked** the new SSRF/metadata material to [[cybersecurity/14-api-security/04-input-validation-and-injection|API security's SSRF note]] — the two now reinforce each other on the Capital One attack path
- [x] **Integration** — 09 README rewritten, root README tree updated. No Quartz symlink (cybersecurity is a directory symlink)
...
- [x] **`01-fundamentals/` 3 notes → 5 + README.** The three existing notes (what cybersecurity is, CIA triad, attacker types) are good and untouched. Added the two concepts they _reference as important but never teach_: **`04-security-principles`** (the durable Saltzer-Schroeder principles — defence in depth, least privilege, fail-safe, zero trust, minimise-surface, no-obscurity — framed as a _reasoning tool_ you derive rules from) and **`05-threat-modeling`** (Shostack's four questions, STRIDE, trust boundaries — the bridge from the attacker taxonomy to a plan). Plus a README the folder lacked
- [x] **`08-GRC/` 2 notes → 4.** The folder was heavy on **R** (risk) and **C** (compliance), light on **G** (governance). Added **`03-governance-and-the-human-layer`** (running security as a _program_ — policy→standard→procedure hierarchy, the CISO/roles, and the human layer: awareness and a _blameless reporting culture_) and **`04-third-party-and-supply-chain-risk`** (the dominant modern breach vector — SolarWinds, Log4Shell, Target; SBOMs, vendor vetting — entirely absent before, and increasingly _the_ risk)
...
**His probe — "no more content gaps? data analysis?" — and he was right.** An honest audit found a genuine hole: the vault had [[data-engineering/index|data engineering]] (build the warehouse) and [[ai-ml/01-data-scientist/index|data science]] (models/inference), but the **enormous middle — the analyst discipline, the most common data job** — was missing. The data-scientist track even _distinguished_ the analyst without covering it. This also swept up the **time-series** gap (nothing on trend/seasonality/forecasting anywhere).

- [x] **`data-analysis/`** — ✅ **12 files (8 notes + README + projects + 2-file interview bank).** The analyst role vs engineer vs scientist → **SQL for analysis** (window functions & CTEs — the primary skill) → metrics & KPIs (vanity vs actionable, **Goodhart**) → exploratory & **diagnostic** analysis (_why did it change?_ as a method) → product analytics (funnels, cohorts, retention, **LTV:CAC**) → **time series** (decomposition, seasonality, forecasting — fills that gap too) → dashboards & **the semantic layer** → **communicating** (the deliverable is a decision)
...
- [x] **Distilled `sources/Developer habits.md`** — ✅ → [[concepts/04-best-practices/07-habits-that-make-change-easier|concepts/04-best-practices/07]], the 7 habits under one through-line (_make the next change easier_), cross-linked to clean-code, SOLID, type-driven design and the anti-corruption-layer pattern
...
- [x] **Thicken `08-GRC` and `01-fundamentals`** — ✅ both expanded by _adding the missing foundational concepts_ rather than padding the (good) existing notes. **Fundamentals 3→5 + README:** added `04-security-principles` (defence in depth, least privilege, fail-safe, zero trust — a reasoning tool) and `05-threat-modeling` (the four questions, STRIDE, trust boundaries) — both were repeatedly _mentioned as important but never taught_. **GRC 2→4:** added `03-governance-and-the-human-layer` (the underweighted 'G' — policy hierarchy, the security program, and security awareness/blameless culture) and `04-third-party-and-supply-chain-risk` (SolarWinds/Log4Shell/Target, SBOMs — the dominant modern vector, absent before)
...
**Still open:** — a course (`01`–`07`) plus `frontend/frameworks/{react,next,css}`, mirroring `backend/`. **The bigger half of that is a _move_, not new writing**: ~44,000 words of React/Next material currently sit in `projects/`, indexed only by `frontend/index`. That's a findability problem, and it's the next batch. Combined 5d+5e: **22 notes, ~29,000 words**, and **no domain README now names exercises as missing.** The standing caveat applies harder than usual to the last one — three of its four notes are organisational disciplines, and a solo project has no dev/ops wall, no ticket queue and no on-call rotation to validate them against.
...
| ~~13~~ | ~~_optional_ — theory of computation, computer architecture, discrete math~~ ✅ | 28    |
