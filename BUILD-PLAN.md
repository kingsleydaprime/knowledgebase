...
**`frontend/` is now a course + `frameworks/`, mirroring `backend/`.** Both moves were already prescribed by the vault: `concepts/README` said _"02-frontend arguably belongs in frontend/. Not moved yet"_, and `frontend/README` said _"distil the recurring material and leave the war stories in projects/, cross-linked — the same split backend uses."_
...
- [x] **Integration** — ✅ root README structure tree, `INTERVIEW.md` (fifteen → sixteen banks), `build-your-own-shit/README` (thirteen → sixteen), and two Quartz symlinks
...
- [x] **`languages/08-swift/`, `09-kotlin/`, `10-dart/`** — ✅ **29 files, ~19,600 words.** Kotlin is deliberately the shortest of the three because [[languages/01-java/README|the Java course]] already covers the JVM underneath it; Dart is shortest overall because it holds few surprises for anyone who knows Java or TypeScript
- [x] **A correction carried into the build** — ✅ he said the mobile languages were "swift, dart, java, js". **Android is Kotlin, not Java** — Google went Kotlin-first in 2019 — so the track is Swift/Kotlin/Dart, with TypeScript already covered by [[frontend/frameworks/react/README|React]]
- [x] **Integration** — ✅ `languages/README` (the stale _"Kotlin would slot in if notes get written"_ line is now redundant and removed), root README tree, `INTERVIEW.md` (sixteen → seventeen), `project-ideas.md`, the Quartz landing page (six → **seven columns**), and a `mobile` symlink
...
**From the queue.** Built as `cybersecurity/13-...`, the **defensive twin of [[cybersecurity/11-binary-exploitation/README|binary exploitation]]** — understanding programs rather than breaking them — completing the offensive-technical trio (11 exploitation, 12 AD, 13 RE/malware).
...
- [x] **Integration** — cybersecurity/README (new section), root README tree, `cybersecurity/projects.md` (two reps: a crackme, then a lab-isolated sample + YARA rule). No Quartz symlink (cybersecurity is a directory symlink)
...
- [x] **Deliberately non-duplicative** — it overlaps [[cybersecurity/04-web-security/README|web-security]] and [[backend/05-auth/README|backend/auth]], so it covers only what's _API-specific_ and cross-links the general treatment for injection, OAuth and TLS. Positioned as the attacking-and-defending twin of [[backend/02-api-design/README|API design]]
- [x] **Integration** — cybersecurity/README (new section), root README tree, `cybersecurity/projects.md` (a BOLA/BFLA rep on crAPI), and cross-links added _into_ web-security, backend/api-design and backend/auth (so the building-side notes point at the security-side). No Quartz symlink (cybersecurity is a directory symlink)
