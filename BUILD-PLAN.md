...
**His call, and the right one:** the vault had ~1M words of map and one 216-line file of territory. *"I want my courses to make me really good"* — which is a reps problem, not a notes problem.
...
- [x] **A *done when* on every project** — ✅ the convention borrowed from `build-your-own-shit`'s "works when". **A project without a finish line gets abandoned or gold-plated**, and the old list had neither
- [x] **`project-ideas.md` rewritten as an index** — ✅ 216 → 94 lines. A one-per-domain table naming *the one to do first*, the "if you only do five", and the two sections that are career-target rather than domain-scoped (Java/systems, build-your-own)
...
- [x] **Integration** — ✅ `languages/README` (the stale *"Kotlin would slot in if notes get written"* line is now redundant and removed), root README tree, `INTERVIEW.md` (sixteen → seventeen), `project-ideas.md`, the Quartz landing page (six → **seven columns**), and a `mobile` symlink

**The load-bearing note in the mobile track is 03, the lifecycle.** Everything else — view models, saved-state handles, offline-first databases, resumable navigation — is a response to *the OS can destroy your process at any moment with no reliable warning.* Its testing section is the part most developers never do, and the interview bank says plainly that the process-death questions are what separate people who've shipped from people who've followed tutorials.

**And the honest labelling holds:** both tracks are `[reference]`. Nothing in `mobile/` has been shipped to a store by its author, and `mobile/projects.md` says so in its first line. The cheapest fix is its first rung — *survive process death* — which is an afternoon.
...
**From the queue.** Built as `cybersecurity/14-...`, positioned as *"web security minus the browser"* and structured around the **OWASP API Security Top 10 (2023)** as its backbone.
...
- [x] **Deliberately non-duplicative** — it overlaps [[cybersecurity/04-web-security/README|web-security]] and [[backend/05-auth/README|backend/auth]], so it covers only what's *API-specific* and cross-links the general treatment for injection, OAuth and TLS. Positioned as the attacking-and-defending twin of [[backend/02-api-design/README|API design]]
- [x] **Integration** — cybersecurity/README (new section), root README tree, `cybersecurity/projects.md` (a BOLA/BFLA rep on crAPI), and cross-links added *into* web-security, backend/api-design and backend/auth (so the building-side notes point at the security-side). No Quartz symlink (cybersecurity is a directory symlink)
...
- [x] **`cybersecurity/09-cloud-security/` — 1 note → 5.** The old single note was genuinely good but undersold the topic, so I split-and-deepened rather than rewrote: shared responsibility model → **identity-is-the-perimeter** (IAM depth, least privilege, and the *privilege-escalation paths* over-permissioning enables — the same "permissions that reach permissions" idea as API BOLA) → **the attack surface** ranked (public buckets #1, then the metadata-service-via-SSRF path, tied explicitly to Capital One and my new API-security SSRF note) → **container/k8s** (the three layers, every default insecure) → **cloud-native defence** (shift-left, CSPM/CIEM/CNAPP, CloudTrail→SIEM, DevSecOps)
