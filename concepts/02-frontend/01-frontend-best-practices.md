...
- **Prop drilling awareness** — passing data down through several intermediate components that don't themselves use it, purely to reach a distant descendant, is a sign that data might belong in shared state instead (see [[02-state-management|state-management]]) — though as covered there, reaching for global state before this genuinely becomes painful is its own, opposite mistake.
